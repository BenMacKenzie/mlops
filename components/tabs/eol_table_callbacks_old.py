...
import dash_bootstrap_components as dbc
from utils.db_universal import get_eol_definitions, create_eol_definition, delete_eol_definition, get_eol_definition_by_name, update_eol_definition, get_project_by_id, sqlQuery
import yaml, json
import yaml
...
def register_eol_callbacks(app):
    ...
    def auto_populate_first_eol(form_store, store_data):
        ...
        print(f"=== AUTO_POPULATE_FIRST_EOL ===")
        print(f"form_store: {form_store}")
        
        if not form_store:
            print("No form_store - no update")
            return no_update, no_update, no_update
        
        # Don't interfere if this was just created by create button
        if form_store.get('just_created', False):
            print("Just created - no update to avoid interference")
            return no_update, no_update, no_update
        
        # Don't interfere if old_name matches a newly created item name pattern
        old_name = form_store.get('old_name')
        if old_name and (old_name == 'new' or old_name.startswith('new')):
            print(f"Detected newly created item with name '{old_name}' - no update to avoid interference")
            return no_update, no_update, no_update
        
        # If old_name is None, clear the form
        if not old_name:
            print("No old_name - clearing form")
            return '', '', ''
        ...
        eol_def = get_eol_definition_by_name(old_name, current_project_id)
    ...
    def update_eol_definitions(store_data, save_clicks, delete_clicks, active_tab, form_store_input,
        ...
        if ctx.triggered:
            ...
            if trigger_id == 'update-eol-button' and name and sql_def:
                ...
                try:
                    ...
                    if proj is not None:
                        ...
                        view_name = name
                        # Build and execute DDL for view in Databricks (ML data source)
                        ddl = f"CREATE OR REPLACE VIEW {catalog}.{schema}.{view_name} AS {sql_def}"
                        print(f"DEBUG: Creating view with DDL: {ddl}")
                        # Import Databricks-specific sqlQuery for view creation
        ...
        # If none found, clear the form store
        if eol_df.empty:
            return [html.P("No EOL definitions found for this project.")], {'old_name': None, 'create_mode': False}, None
        # Build list items and dropdown options
        eol_items = []
        # Get the currently selected EOL name from form store (prefer input over state)
        current_form_store = form_store_input if form_store_input else new_form_store
        selected_eol = current_form_store.get('old_name', None) if current_form_store else None
        ...
        # 3. NOT in create mode (which should keep form clear)
        ctx = callback_context
        in_create_mode = current_form_store.get('create_mode', False) if current_form_store else False
        # Only auto-select after save/delete operations, not on initial load
        should_auto_select = (
            selected_eol is None and 
            not eol_df.empty and 
            not in_create_mode and  # Don't auto-select when in create mode
            operation_performed in ['save', 'delete']  # Only after successful update/delete operations
        )
        
        if should_auto_select:
            selected_eol = eol_df.iloc[0]['name']
            new_form_store = {'old_name': selected_eol, 'create_mode': False}
        elif in_create_mode:
            # Preserve create_mode if it was set
            new_form_store = {'old_name': None, 'create_mode': True}
        
        for idx, (_, row) in enumerate(eol_df.iterrows()):
            # Highlight the selected item by name (now unique)
    ...
    def populate_eol_form(eol_clicks, store_data, form_store):
        ...
        if trigger_obj.get('type') == 'eol-list-item':
            ...
            if eol_def is not None:
                ...
                return name_val, sql_val, label_val, {'old_name': eol_name, 'create_mode': False, 'just_created': False}
    ...
    @app.callback(
        [Output('eol-name-input', 'value', allow_duplicate=True),
         ...
         Output('eol-label-input', 'value', allow_duplicate=True),
         Output('eol-form-store', 'data', allow_duplicate=True),
         Output('eol-definitions-list', 'children', allow_duplicate=True)],
        Input('create-eol-button', 'n_clicks'),
        State('list-store', 'data'),
        prevent_initial_call=True
    )
    def new_eol_definition(n_clicks, store_data):
        """Create a new EOL definition with default values."""
        if not n_clicks:
            return no_update, no_update, no_update, no_update, no_update
            
        # Get current project ID
        project_id = store_data.get('active_project_id') if isinstance(store_data, dict) else None
        if not project_id:
            print("No active project for new EOL definition")
            return no_update, no_update, no_update, no_update, no_update
            
        # Create a new EOL definition with a unique name
        # Check how many "new" definitions already exist and create a unique name
        df = get_eol_definitions(project_id)
        existing_names = df['name'].tolist() if not df.empty else []
        
        # Find next available "new" name
        counter = 1
        new_eol_name = "new"
        while new_eol_name in existing_names:
            counter += 1
            new_eol_name = f"new{counter}"
        ...
        if not success:
            print("Failed to create new EOL definition")
            return no_update, no_update, no_update, no_update, no_update
        
        # Refresh the EOL definitions list
        eol_df = get_eol_definitions(project_id)
        
        # Create list items with the new EOL selected
        eol_items = []
        for idx, (_, row) in enumerate(eol_df.iterrows()):
            is_active = (new_eol_name == row['name'])  # Highlight the newly created one
            eol_items.append(
                dbc.ListGroupItem(
                    row['name'], id={"type": "eol-list-item", "index": row['name']},
                    action=True, active=is_active
        ...
        # Return form values, form store, and refreshed list
        return (new_eol_name, "SELECT 1 as placeholder", "", 
                {'old_name': new_eol_name, 'create_mode': False, 'just_created': True},
                [list_group])